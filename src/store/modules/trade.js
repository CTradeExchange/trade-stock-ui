import { queryPositionPage, queryHistoryCloseOrderList, queryPBOOrderPage, queryAbccPboPage, queryOrderPage, tradeRecordList, queryStockPendingOrderList } from '@/api/trade'
...
export default {
    ...
    getters: {
        ...
        pendingPriceRang (state, getters, rootState) {
            ...
            const digits = product.price_digits || product.symbolDigits
            const tradeType = product.tradeType
            const point = Math.pow(0.1, digits)
            const pip = point * product.pointRatio
            ...
            const defaultBuyPrice = tradeType === 6 ? product.cur_price : minus(buy_price, priceMinLimitPrice) // 买入方向挂单价格默认值
            const defaultSellPrice = tradeType === 6 ? product.cur_price : plus(sell_price, priceMinLimitPrice) // 卖出方向挂单价格默认值
            return {
    ...
    mutations: {
        ...
        Update_positionList (state, { tradeType, list }) {
            ...
            list && list.forEach(item => {
                ...
                if (curPosition) {
                    Object.assign(item, {
                        profitLoss: curPosition.profitLoss,
                        profitLossPercent: curPosition.profitLossPercent
                    })
                } // 如果map数据中已经有此持仓信息，将之合并到item
        ...
        Update_pendingList (state, { tradeType, list }) {
            // if (state.pendingList[tradeType]) {
            //     list = list.concat(state.pendingList[tradeType])
            // }

            vue_set(state.pendingList, tradeType, list)
            const pendingMap = state.pendingMap
            list.forEach(item => {
                const orderId = Number(tradeType) === 6 ? item.orderId : item.id
                if (!item || !orderId) return false
                const key = `${orderId}_${item.tradeType}`
        ...
        Update_positionProfitLossList (state, { tradeType, list }) {
            ...
            const positionMap = state.positionMap
            list.forEach(({ positionId, profitLoss, previewStopPrice, profitLossPercent }) => {
                ...
                if (position) {
                    ...
                    vue_set(positionMap, key, { profitLoss, profitLossPercent })
                }
            })
        }
    },
    actions: {
        ...
        queryPBOOrderPage ({ dispatch, commit, state, rootState }, params) {
            ...
            if (Number(tradeType) === 9) {
                ...
                return queryStockPendingOrderList({
                    tradeType: 6,
                    customerNo: rootState._user.customerInfo.customerNo
                }).then(res => {
                    if (res.check()) {
                        if (res.data.length > 0) {
                            const list = res.data
                ...
                /* return queryOrderPage({
                    ...
                    if (res.check()) {
                        ...
                        commit('Update_pendingList', { tradeType, list: res.data.records })
                    }
                    return res
                }) */
            } else {
        ...
        tradeRecordList () {},
        // 清除预埋单列表
        clearPBOOrderPage ({ dispatch, commit, state, rootState }, tradeType) {
            state.pendingList[tradeType] = []
        }
    }
}
