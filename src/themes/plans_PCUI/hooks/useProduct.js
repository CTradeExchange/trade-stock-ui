    ...
    const productList = computed(() => {
        ...
        if (unref(tradeType) !== '6') {
            arr.sort((a, b) => {
                ...
                } else if (sortField.value === 'rolling_upDownWidth') { // 24H涨跌幅
                ...
                } else if (sortField.value === 'upDownWidth') { // 涨跌幅
                    const firtstValue = parseFloat(firstEl[sortField.value]) || defaultInfinity
                    const secondValue = parseFloat(secondEl[sortField.value]) || defaultInfinity
                    return firtstValue - secondValue
                } else if (sortField.value === 'upDownAmount') { // 涨跌额
                    const firtstValue = parseFloat(firstEl[sortField.value])
                    const secondValue = parseFloat(secondEl[sortField.value])
                    return firtstValue - secondValue
...
/* -------股票玩法的产品列表排序功能 start-------- */
export const productStockListSet = (productList) => {
    const list = productList.slice(0)
    const defaultInfinity = sortType.value === 'asc' ? Infinity : -Infinity
    ...
    list.sort((a, b) => {
        ...
        if (field === 'symbolName') {
            ...
            if (parseFloat(firstEl['cur_price']) && parseFloat(secondEl['cur_price'])) {
                return firstEl[field].localeCompare(secondEl[field])
        ...
        } else if (field === 'cur_price') {
            const firtstValue = firstEl[field] || defaultInfinity
            const secondValue = secondEl[field] || defaultInfinity
            return gte(firtstValue, secondValue) ? 1 : -1
        } else if (field === 'upDownWidth') {
            const firtstValue = isNaN(parseFloat(firstEl[field])) ? defaultInfinity : parseFloat(firstEl[field])
            const secondValue = isNaN(parseFloat(secondEl[field])) ? defaultInfinity : parseFloat(secondEl[field])
            return firtstValue - secondValue
        } else if (sortField.value === 'upDownAmount') { // 涨跌额
            const firtstValue = parseFloat(firstEl[sortField.value])
            const secondValue = parseFloat(secondEl[sortField.value])
