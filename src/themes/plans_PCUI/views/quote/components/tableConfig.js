...
import ProductLabel from '@planspc/components/productLabel'
...
export const getColumns = tradeTypeValue => {
    ...
    const refreshSort = inject('refreshSort')
    ...
    // 手动排序
    const handSort = (value) => {
        sortFunc(value)
        if (parseInt(tradeTypeValue) === 6) refreshSort(value)
    }

    // 列表label的名称
    const headerCommonName = ({ row }) => (
        <>
            <span class="pointer" onclick={() => handSort('symbolName')}>
                {t('trade.name')}
    ...
    // 股票列表label的最新价
    const headerCommonLasPriceStock = ({ row }) => (
        <>
            <span class="pointer" onclick={() => handSort('cur_price')}>
                {t('trade.newPrice')}
                <SortIcon name='cur_price' sort-field={sortField.value} sort-type={sortType.value} />
            </span>
        </>
    )
    ...
    // 股票列表label的涨跌额
    const headerCommonUpDownAmountStock = ({ row }) => (
        <>
            <span class="pointer" onclick={() => handSort('upDownAmount')}>
                {t('chart.diffPrice')}
                <SortIcon name='upDownAmount' sort-field={sortField.value} sort-type={sortType.value} />
            </span>
        </>
    )
    ...
    // 股票列表label的涨跌幅
    const headerCommonUpDownWidthStock = ({ row }) => (
        <>
            <span class="pointer" onclick={() => handSort('upDownWidth')}>
                {t('chart.quoteChange')}
                <SortIcon name='upDownWidth' sort-field={sortField.value} sort-type={sortType.value} />
            </span>
        </>
    )

    // 产品标签
    const createProductLabel = (row) => (
        <div className='smllPic'>
            <ProductLabel symbolKey={row.symbolId + '_' + row.tradeType}></ProductLabel>
            <span className='code'> {row.symbolCode}</span>
            {row.enableLongLoan === 1 ? <img class='icon-loan rong' src="/images/rong.png?v=1" /> : '' }
            {row.enableShortLoan === 1 ? <img class='icon-loan kong' src="/images/kong.png?v=1" /> : '' }
            {row.dividend === 1 ? <img class='icon-loan chu' src="/images/chu.png?v=1" /> : '' }
        ...
        6: [
            {
                ...
                formatter: row => (<div class='chunk'>
                    ...
                    <div>
                        <span class='name' style={{ 'margin-right': '3px' }}>
                            { row.symbolName }
                        </span>
                        {createProductLabel(row)}
                    </div>
                </div>),
                slots: {
                    header: headerCommonName
            ...
            },
            {
                ...
                slots: {
                    header: headerCommonLasPriceStock,
                }
            },
            {
                name: t('chart.diffPrice'),
                align: 'left',
                minWidth: 160,
                formatter: row => (
                ...
                slots: {
                    header: headerCommonUpDownAmountStock
                }
            },
            {
                name: t('chart.quoteChange'),
                align: 'left',
                minWidth: 160,
                className: 'upDownWidth',
                formatter: row => (
                ...
                slots: {
                    header: headerCommonUpDownWidthStock,
                }
            },
            {
                name: t('trade.highPrice'),
