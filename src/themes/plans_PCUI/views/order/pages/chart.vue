    ...
    <div class='symbol-info'>
        ...
        <div class='item range'>
            ...
            <p>
                ...
                <span v-if='[6].includes(product?.tradeType)' :class='product.upDownColor'>
                    {{ product.upDownWidth }}
                </span>
                <span v-if='[6].includes(product?.tradeType)' :class='product.upDownColor'>
                    {{ product.upDownAmount }}
                </span>
                <span v-else :class='product?.rolling_upDownColor'>
        ...
        <div class='item collect'>
            <i class='icon icon_zhixingdiaodu' :title='$t("route.corporateAct")' @click='$router.push(corporateActRoute)'></i>
            <i
                v-preventReClick
                class='icon'
                :class="[isOptional ? 'icon_hangqingliebiaoyijiazixuan' : 'icon_zixuankongxin']"
                :title='$t("trade.addToOptional")'
                @click='addOptional'
            ></i>
            <i v-if='[1, 2, 6].includes(product?.tradeType)' class='icon icon_heyuexiangqing' :title='$t("route.contract")' @click='$router.push(contractRoute)'></i>
        </div>
    </div>
    <CorporateActNotice />
    <div class='tv-head'>
...
export default {
    components: { tv, KIcon, EtfIcon, StudyList, CorporateActNotice },
    setup () {
        ...
        // 公司行动路由
        const corporateActRoute = computed(() => (`${route.path}/corporateAct?symbolId=${product.value?.symbolId}&tradeType=${product.value?.tradeType}`))
        ...
        const changeRoute = () => {
            ...
            QuoteSocket.deal_subscribe(product.value.symbolId, 1, curDigits, product.value.tradeType, 1) // 该页面因为要实时更新成交量，所以改成订阅deal_subscribe成交记录显示最新价
            // const invertColor = localGet('invertColor')
            initChartData()
            chartRef.value && chartRef.value.reset({
                initialValue: initialValue.value,
