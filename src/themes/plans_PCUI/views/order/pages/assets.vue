<template>
    <section v-if='$store.state._user.customerInfo' class='assetsModule'>
        ...
        <!-- 合约逐仓 -->
        <div v-else-if='product.tradeType===6' class='rightActions'>
            <div class='assetItem'>
                <p>{{ $t('assets.stock.USD') }}{{ $t('assets.assetsEquity') }} (USD)</p>
                <p class='amount'>
                    {{ accountTradeType6?.USD?.netAsset || '--' }}
                </p>
            </div>
            <div class='assetItem'>
                <p>{{ $t('assets.stock.HKD') }}{{ $t('assets.assetsEquity') }} (HKD)</p>
                <p class='amount'>
                    {{ accountTradeType6?.HKD?.netAsset || '--' }}
                </p>
            </div>
            <van-button v-if='$store.state._base.plans.length>1' class='btn' type='primary' @click="$router.push($route.path+'/transfer?tradeType='+product.tradeType)">
                {{ $t('trade.transfer') }}
...
export default {
    setup () {
        ...
        const accountTradeType6 = computed(() => store.state._user.accountAssets['6'])
