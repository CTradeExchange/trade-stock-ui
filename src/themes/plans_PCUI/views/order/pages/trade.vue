<template>
    ...
    <div class='trade-wrap'>
        <div class='buy-wrap'>
            ...
            <!-- 股票的可买可卖等数据 -->
            <StockSummary
                v-if='estimateStockNum && product.tradeType===6'
                :data='estimateStockNum.buy'
                direction='buy'
                :order-type='orderType'
                :pending-price='buy.pendingPrice'
                :product='product'
                :volume='buy.volume'
            />
            <!-- 订单金额 -->
            <Assets
                v-else-if='product.tradeType!==6 && account.buy'
                :data='estimateStockNum'
                direction='buy'
                :order-type='orderType'
                :pending-price='buy.pendingPrice'
                :product='product'
            ...
            <div class='footerBtn buy'>
                <van-button block :disabled='buy.loading || product.tradeEnable === 2' :loading='buy.loading' size='normal' @click='submitHandlerClick("buy")'>
                    {{ $t('trade.buyText') }}
                </van-button>
            </div>
        ...
        <div class='sell-wrap'>
            ...
            <StockSummary
                v-if='estimateStockNum && product.tradeType===6'
                :data='estimateStockNum.sell'
                direction='sell'
                :order-type='orderType'
                :pending-price='sell.pendingPrice'
                :product='product'
                :volume='sell.volume'
            />
            <!-- 可用余额 -->
            <Assets
                v-else-if='product.tradeType!==6 && account.sell'
                :account='account.sell'
                direction='sell'
                :product='product'
                :volume='sell.volume'
            />
            ...
            <div class='footerBtn sell'>
                <van-button block :disabled='sell.loading || product.tradeEnable === 2' :loading='sell.loading' size='normal' @click='submitHandlerClick("sell")'>
                    {{ $t('trade.sellText') }}
                </van-button>
            </div>
            <LoginMask class='loginMaskPop' />
            ...
            <OrderConfirmDialog
                ...
                :can-close-position-num='estimateStockNum?.canClosePositionNum'
                :direction='submitType'
                :loan-amount='loanAmount'
                :order-type='orderType'
                :pending-price="submitType === 'buy' ? buy.pendingPrice : sell.pendingPrice"
                :product='product'
                :volume="submitType === 'buy' ? buy.volume : sell.volume"
...
import { addMarketOrder, stockCheckOrder, stockCalculateOrder } from '@/api/trade'
...
export default {
    ...
    setup (props, { emit }) {
        ...
        const state = reactive({
            ...
            estimateStockNum: null,
            loanAmount: 0, // 融资金额
            showConfirmDialog: false // 是否显示下单确认弹窗
        })
        ...
        // 点击提交按钮
        const submitHandlerClick = (type) => {
            state.submitType = type
            const params = orderParams()
            if (!params) return
            if (params.tradeType === 6) {
                return stockCheckOrderHandler(params)
            } else {
        ...
        // 股票玩法下单验证
        const stockCheckOrderHandler = (params) => {
            const direction = state.submitType
            if (state[direction].loading) return false
            state[direction].loading = true
            return stockCheckOrder(params).then(res => {
                ...
                // 显示下单确认弹窗
                state.loanAmount = Number(res.data.loanAmount)
        ...
        // 确认下单
        const confirmHandle = () => {
            submitHandler(state.submitType)
        }

        // 点击提交按钮
        const submitHandler = (direction) => {
            state.submitType = direction
            const params = orderParams()
            if (!params) return
            if (state[direction].loading) return false
            state[direction].loading = true
            addMarketOrder(params)
                .then(async res => {
                    ...
                    // 刷新持仓列表
                    if ([1, 2, 6].includes(product.value?.tradeType)) {
                    ...
                    // 清除表单
                    resetForm()
                    state[direction].volume = ''
                    state[direction].pendingPrice = ''
                    Toast({
                        message: [1, 12].includes(params.bizType) ? t('trade.orderSuccessToast') : t('trade.orderPendingSuccessToast'),
                        duration: 1000,
                        forbidClick: true,
                    })
                    if (product.value?.tradeType === 6) {
                        launchEstimateAmount('buy')
                        launchEstimateAmount('sell')
                    }
                })
                .catch(err => {
                    state[direction].loading = false
        ...
        const queryAccountInfo = () => {
            if ([1, 2, 6].includes(product.value?.tradeType)) return
        ...
        const init = () => {
            ...
            store.dispatch('_quote/querySymbolInfo', { symbolId, tradeType }).then(product => {
                ...
                if (product.tradeType === 6 && account.value.buy) {
                    launchEstimateAmount('buy')
                    launchEstimateAmount('sell')
                }
            }).then(() => {
        ...
        // 获取股票预估手续费
        const launchEstimateAmount = (direction) => {
            const [symbolId, tradeType] = symbolKey.value.split('_')
            const price_digits = product.value.hasOwnProperty('price_digits') ? product.value.price_digits : product.value.symbolDigits
            const directionVal = direction === 'sell' ? 2 : 1
            stockCalculateOrder({
                tradeType: parseInt(tradeType),
                accountId: account.value[direction].accountId,
                accountCurrency: account.value[direction].currency,
                accountDigits: account.value[direction].digits,
                direction: directionVal,
                symbolId: Number(symbolId),
                digits: price_digits,
            }).then(res => {
                if (res.check()) {
                    if (!state.estimateStockNum) state.estimateStockNum = {}
                    state.estimateStockNum[direction] = res.data
                }
            })
        }

        // 挂单成交后下发通知更新下单页面的预估最大可买等字段
        const refreshAsset = () => {
            if (product.value?.tradeType === 6 && account.value.buy) {
                launchEstimateAmount('buy')
                launchEstimateAmount('sell')
            }
        }
        ...
        onMounted(() => {
            ...
            document.body.addEventListener('GotMsg_RefreshAsset', refreshAsset, false)
        })

        onBeforeUnmount(() => {
            document.body.removeEventListener('update:multipLeVal', removeEventListener)
            document.body.removeEventListener('GotMsg_RefreshAsset', refreshAsset, false)
        })
