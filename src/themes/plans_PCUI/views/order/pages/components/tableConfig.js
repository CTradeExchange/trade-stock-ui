...
import { closePboOrder, closeTradePboOrder, stockCancelOrder } from '@/api/trade'
...
export const getPendingColumns = tradeType => {
    ...
    // 股票撤单
    const cancelOrderStock = (product) => {
        console.log(product)
        let message, allDeal
        ...
        Dialog.confirm({
            ...
            if (allDeal === 0) closePendingOrder(product)
        }).catch(() => {})
    }
    ...
    const handleCancelOrder = (row) => {
        if (parseInt(row.tradeType) === 6) {
            return cancelOrderStock(row)
        }
    ...
    const columnsMap = computed(() => ({
        ...
        6: [
            ...
            {
                name: t('trade.orderStatus'),
                prop: 'direction',
                align: 'right',
                minWidth: 100,
                formatter: row => <span >
                    { tm('stockPendingStatus')[row.status] }
                </span>
            },
            {
                name: t('trade.pendingUnit'),
                prop: 'requestNum',
                align: 'right',
                minWidth: 120,
                // formatter: row => row.requestNum + `(${row.numberStatisticMode === 2 ? row.accountCurrency : t('trade.volumeUnit')})`
            ...
            { name: t('trade.trustPrice'), prop: 'requestPrice', align: 'right', minWidth: 100 },
            {
                ...
                minWidth: 100,
                formatter: row => (
                    <span class={currentProduct(row)?.cur_color}>
                        {currentProduct(row)?.cur_price}
                    </span>
                )
            },
            { name: t('trade.pendingTime'), align: 'right', minWidth: 180, formatter: row => formatTime(row.orderTime) },
            { name: t('trade.pendingNo'), prop: 'orderId', align: 'right', minWidth: 100 },
            {
                name: t('c.handle'),
                fixed: 'right',
                align: 'right',
...
export const getTransactionColumns = (tradeType) => {
    ...
    const columnsMap = computed(() => ({
        ...
        6: [
            ...
            {
                name: t('trade.dealPrice'),
                prop: 'executePrice',
                align: 'right',
                minWidth: 120,
            },
            { name: t('trade.dealAmount'), prop: 'tradeAmount', align: 'right', minWidth: 120, },
            { name: t('trade.fee'), prop: 'commission', align: 'right', minWidth: 120 },
            {
                name: t('trade.dealTime'),
                align: 'right',
...
export const getAssetColumns = (tradeType) => {
    ...
    const toProduct = (row) => {
        ...
        router.replace(`/order?symbolId=${row.symbolId}&tradeType=${row.tradeType}`)
    }
    ...
    const columnsMap = computed(() => ({
        ...
        6: [
            ...
            },
            {
                ...
                minWidth: 150,
                formatter: row => {
                    return <span >{row.symbolCode}</span>
                }
            },
            {
                name: t('trade.profit'),
                ...
                minWidth: 150,
                formatter: row => {
                    return <span className={`${parseFloat(row.profitLoss) > 0 ? 'riseColor' : ''} ${parseFloat(row.profitLoss) < 0 ? 'fallColor' : ''}`}>{row.profitLoss}</span>
                }
            },
            {
                name: t('trade.direction'),
            ...
            {
                name: t('trade.marketValue'),
                prop: 'openPrice',
                align: 'right',
                minWidth: 100,
                formatter: row => {
                    return <span>{ getMarketValue(row, currentProduct(row)) }</span>
                }
            },
            {
                name: t('trade.currentPrice'),
            ...
            },
            {
                ...
                slots: {
                    default: ({ row, onGetComponentRefs }) => {
                        ...
                        return (
                            <>
                                <span className='link' onclick={toProduct.bind(null, row)} type='text'>
                                    { t('header.trade') }
                                </span>
                            </>
                        )
