...
export default function hooks (state) {
    ...
    const account = computed(() => {
        ...
        if ([1, 2].includes(tradeType)) {
            ...
            let curAccount;
            if(us) curAccount = accountList.find(el => (el.tradeType === tradeType && el.currency==='USD'))
            else if(hk) curAccount = accountList.find(el => (el.tradeType === tradeType && el.currency==='HKD'))
            account = {
                buy: curAccount,
                sell: curAccount,
            }
    ...
    const bizType = computed(() => {
        ...
        if([1,2].includes(tradeType)){
            ...
            if(state.orderType===1){
                bizType = state.submitType === 'buy' ? 1:2;
            }else{
                bizType = state.submitType === 'buy' ? 12:13;
            }
