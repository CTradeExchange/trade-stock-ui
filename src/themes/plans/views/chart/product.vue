<template>
    <div class='page-wrap' :class='{ isIframe: isUniapp }'>
        <LayoutTop v-if='!isUniapp' :back='true' :menu='false'>
            <p class='symbolName'>
                <!-- <i v-if='product?.symbolName' class='icon_quanrejiaohuan- sortIcon' @click='showSidebar=true'></i> -->
                <!-- <i v-if='product?.symbolName' class='icon_chouti1' @click='showSidebar=true'></i> -->
                {{ product?.symbolName }}
                <sup v-show='isHK || isUS' class='supLabel' :class='{ hk:isHK,us:isUS }'>
                    {{ isUS ? 'US':'HK' }}
                </sup>
            </p>
            <p v-if='product.tradeType===6'>
                <span>{{ product.symbolCode }}</span>
                <span class='tradeStatus'>
                    {{ product.stockStatus==='CLOSED' ? $t("trade.stockStatusClosed") : $t("trade.stockStatusTrading") }}
                </span>
            </p>
            <!-- <p class='infomation'>
            ...
            <template #right>
                <div class='right-wrap'>
                    <div class='ft'>
                        <button v-preventReClick class='collectIcon' :class='{ collected: isSelfSymbol }' @click='addOptional'>
                            <i
                                ref='collect'
                        ...
                        <span
                            v-if='![3, 5, 9].includes(product?.tradeType)'
                            @click='toContractInfo'
                        >
                            <i class='icon_guanyu1'></i>
                        </span>
                        <span @click='showSidebar=true'>
        ...
        <div v-if='product && product.tradeEnable===1' class='footerBtnBoxWarp'>
...
import CorporateActNotice from './components/corporateAct/notice'
import CorporateActList from './components/corporateAct/list'
...
export default {
    components: { KIcon, StudyList, tv, StallsAndDeal, Loading, sidebarProduct, realtimeInvestCompose, ProductInfo, CorporateActNotice, CorporateActList },
    setup (props) {
