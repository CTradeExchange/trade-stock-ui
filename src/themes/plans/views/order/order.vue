<template>
    <div class='orderWrap'>
        <div class='orderTop'>
            ...
            <SymbolAccount v-if='product && product.tradeType===6' :product='product' />
            <SwitchTradeType
                :product='product'
        ...
        <div ref='containerRef' class='container'>
            <div v-if='product' class='main'>
                ...
                <div v-if='product' class='right'>
                    ...
                    <!-- 订单金额 -->
                    <Assets
                        v-if='account && product.tradeType!==6'
                        :account='account'
                        :direction='direction'
                        :product='product'
                        :volume='volume'
                    ...
                    <!-- 预估金额 -->
                    <StockSummary
                        v-if='[6].includes(product.tradeType)'
                        class='mtop20'
            ...
            <div v-else-if='product && product.tradeEnable===1' class='footerBtn' :class='[direction]'>
                <van-button block :disabled='loading' :loading='loading' size='normal' @click='submitHandlerClick'>
                    <i class='icon' :class='direction==="buy" ? "icon_mairu" : "icon_maichu"'></i>
                    {{ directionText }}
                </van-button>
            </div>
            <!-- 交易记录 -->
            <OrderRecord ref='roderRecordRef' :account-id='account.accountId' :trade-type='productTradeType' @subscribe='setProductKeys' />
        </div>
...
<script>
import { reactive, toRefs, computed, ref, watch, onBeforeUnmount, onMounted, nextTick } from 'vue'
...
import { debounce, delayAwaitTime } from '@/utils/util'
import SymbolAccount from './components/symbolAccount.vue'
...
export default {
    ...
    setup () {
        ...
        const containerRef = ref(null) // 交易容器
        ...
        watch(() => product.value?.tradeType, val => {
            if ([1, 2, 6].includes(Number(val))) {
                const subParam = {
                    tradeTypes: val
                }
                if (Number(val) === 6) {
                    subParam.subscribeType = '1'
                }
                MsgSocket.subscribeAsset(subParam)
            }
        }, {
            immediate: true
        })
        ...
        watch(
            ...
            () => {
                ...
                if (product.value?.tradeType === 6 && account.value) {
                    launchEstimateAmount()
                }
            },
        )
        ...
        watch(
            ...
            async (newval, oldVal) => {
                await nextTick()
                if (route.name !== 'Order') return false
                containerRef.value.scrollTop = 0
                if (!(newval.symbolId === oldVal.symbolId && newval.tradeType === oldVal.tradeType && newval.direction === oldVal.direction)) {
                    switchProduct(route.query.symbolId, route.query.tradeType).then(() => {
                        state.orderType = 1
                        if (route.query.direction) state.direction = route.query.direction
                        init()
                    })
                }
            }
        ...
        const init = () => {
            ...
            store.dispatch('_quote/querySymbolInfo', { symbolId, tradeType }).then(product => {
                ...
                if (product.tradeType === 6 && account.value) {
                    launchEstimateAmount()
                }
            })
        }
        ...
        const orderParams = () => {
            ...
            // else if (['6'].includes(tradeType)) params.digits = price_digits
            return params
        }

        // 点击提交按钮
        const submitHandlerClick = () => {
        ...
        const submitHandler = (params) => {
            ...
            addMarketOrder(params)
                .then(async res => {
                    ...
                    state.volume = ''
                    if (params.tradeType !== 6) state.pendingPrice = '' // 股票玩法不清除挂单价格
                    Toast({
                    ...
                    queryAccountInfo()
                    if (params.tradeType === 6) launchEstimateAmount()
                })
                .catch(err => {
                    state.loading = false
                })
        ...
        // 获取预估手续费
        const launchEstimateAmount = debounce(() => {
            const [symbolId, tradeType] = symbolKey.value.split('_')
            ...
            stockCalculateOrder({
                ...
                if (res.check()) {
                    state.estimateStockNum = res.data
                }
            })
        })
        ...
        const refreshAsset = () => {
            if (product.value?.tradeType === 6 && account.value) launchEstimateAmount()
        }
