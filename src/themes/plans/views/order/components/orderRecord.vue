    ...
    <div class='module-content'>
        ...
        <div v-if="current === 'pending'">
            ...
            <div v-else class='trust-list'>
                <template v-if='Number(tradeType) === 6'>
                    <trustItemStock v-for='item in pendingList' :key='item.id' :product='item' />
                </template>
                <template v-else>
        ...
        <!-- 成交记录 -->
        <div v-if="current === 'deal'" class='dealListWrapper'>
            <van-empty v-if='dealList.length === 0' class='emptyBox' :description='$t("trade.noDealData")' image='/images/empty.png' />
        ...
        <div v-if="current === 'assets'" class='assets-list'>
            ...
            />
            <van-empty v-if='accountList.length === 0' class='emptyBox' :description='$t("trade.noAssets")' image='/images/empty.png' />
...
import { tradeRecordList, queryStockDealPage } from '@/api/trade'
...
export default {
    ...
    setup (props, { emit }) {
        ...
        function setTabList () {
            let tabList = []
            const accountIds = []
            if ([1, 2, 6].includes(Number(props.tradeType))) {
                tabList = [
        ...
        function getDealRecord () {
            ...
            let api
            if (Number(props.tradeType) === 6) {
                api = queryStockDealPage
            } else {
                api = tradeRecordList
            }
