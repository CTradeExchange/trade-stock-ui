<template>
    ...
    >
        <template #filter>
            ...
            <AccountFilter v-if='Number(tradeType) === 6' :account-id='accountId' @change='changeAccount' />
        </template>
...
export default {
    components: { list, flowFilter, AccountFilter },
    setup (props) {
        ...
        const changeAccount = (val) => {
            requestParams.value.accountId = val
            refresh()
        }
