...
import { positionsTickToObj, stockPositionsTickToObj } from './socketUtil'
...
class SocketEvent {
    ...
    positionsTick (str, tradeType) {
        ...
        // 模拟数据
        // str += '(2022072828134,20,10.4%);(2022072600132,30,1.5%);(2022072600116,60,0.8%)'
        if (this.newPriceTimer) clearTimeout(this.newPriceTimer)
        let curPriceData

        if (Number(tradeType) === 6) {
            curPriceData = stockPositionsTickToObj(str)
        } else {
            curPriceData = positionsTickToObj(str)
        }
    ...
    floatProfitLoss (dataArr, tradeType) {
        ...
        // 股票玩法返回所有币种，合约玩法返回单币种
        const data = Number(tradeType) === 6 ? last.content.assets : last.content
        $store.commit('_user/Update_accountAssets', { tradeType, data })
    ...
    // 消息通知
    notice (data) {
        const { tradeType, updateType, show, bizType, accountId } = data.content
        // 刷新字段：updateType
        // 0 ,"无动作",
        // 1 ,"刷新仓位",
        // 2 ,"刷新挂单",
        ...
        // 拼接查询持仓参数
        const queryPositionParams = {
            tradeType,
            sortFieldName: 'openTime',
            sortType: 'desc'
        }
        if (Number(tradeType) === 6 && !window.isPC) queryPositionParams.accountId = accountId

        if (updateType === 1) {
            store.dispatch('_trade/queryPositionPage', queryPositionParams)
        } else if (updateType === 2) {
            if (Number(tradeType) === 6) { document.body.dispatchEvent(new CustomEvent('GotMsg_RefreshAsset')) }
            ...
            if ([3, 5, 9].includes(Number(tradeType))) store.dispatch('_user/queryCustomerAssetsInfo', { tradeType })
            store.dispatch('_trade/queryPBOOrderPage', {
                tradeType,
                sortFieldName: 'orderTime',
                sortType: 'desc',
    ...
    // 订阅资产数据
    subscribeAsset (params) {
        this.subscribedListAdd(() => {
            this.send('subscribe_asset', params)
        })
    }
