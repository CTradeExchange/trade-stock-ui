...
export const getQuoteService = () => {
    ...
    if (token) {
        quoteService = quoteService.replace('/quote', '/quote_stock')
        quoteService += '?token=' + token
    } else {
        // quoteService = quoteService.replace('/quote', '/quote_stock')
        quoteService = 'wss://nbstream.binance.com/market?uuid=5b00e8f8-e2d7-4fee-a55e-c7eb37dac9bc&lang=zh-cn&clienttype=web'
    }
    return quoteService
}
...
function msgWSEvent (msgWS) {
    ...
    msgWS.addEventListener('message', evt => {
        ...
        else if (typeof evt.data === 'string' && evt.data.startsWith('s')) return MsgSocket.positionsTick(evt.data, 6)
