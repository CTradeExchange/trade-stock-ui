...
// 股票玩法持仓浮动盈亏tick字符串转Object对象
export function stockPositionsTickToObj (str) {
    //消息以-- 分割，[0]是资产消息  [1]是持仓盈亏消息
    //s(HKD,8888.00,8188.00,666.00,600.00,100.00,88.00,50.00,51.00,1);(USD,8888.00,8188.00,666.00,600.00,100.00,88.00,50.00,51.00,1)--(10001,7.45,5.23%);(10002,7.45,5.23%)
    const [accountStr,positionStr] = str.split('@')
    let accountData = {}
    let positionsProfitLoss = {}

    //处理资产数据
    if(accountStr && accountStr !== 's'){
        accountStr.split(';').forEach(el=>{
            if(el.includes('s(')) el = el.match(/\((.+)\)/)[1]
            if(el.startsWith('(')) el =  el.replace(/\(|\)/g, '')
            const accountArr = el.split(',')
            const currency = accountArr[0]
            // 币种的综合资产净值,账户资产净值,证券市值,现金余额,持仓盈亏,现金可提,最大日购买力,最大隔夜购买力,风险状态
            accountData[currency] = {
                conNetAsset: accountArr[1],
                netAsset: accountArr[2],
                stockAsset:accountArr[3],
                balance :accountArr[4],
                profitLoss : accountArr[5],
                cash :accountArr[6],
                maxBuy :accountArr[7],
                maxOvernight : accountArr[8],
                risk : accountArr[9]
            }
    ...
    if(positionStr){
        //仓位id,浮动盈亏,浮动盈亏百分比
        positionsProfitLoss = positionStr.split(';').map(el=>{
